import { ClientOffer, ReactSimplePeerState, ReactSimplePeerStatusState, Status } from '../';
import { handlePeerConnection }                                                  from './handlePeerConnection';
import { send }                                                                  from '../Commands/Tools/send';

export function processClientOffer(data: ClientOffer, id: string, roomCreator: boolean, emitterPeerId: string) {
    ...
    let retry = async () => {
        let relayPeer = Array.from(ReactSimplePeerState.value.connections.values()).find(peer => {
            return peer.peers.includes(emitterPeerId)
        });

        if (!relayPeer || (relayPeer && !await send(relayPeer.model.connection, 'TELL_HIM_TO_RETRY___', emitterPeerId))) {
