...
export function handlePeerConnection(pc: Instance, openConnectionAsInitiator: boolean) {
    pc.on('connect', async () => {
        if (openConnectionAsInitiator) {
            await send(pc, 'OPEN_CNTS_AS_INIT___', JSON.stringify(ReactSimplePeerState.value.peers));
        }
        let state = ReactSimplePeerState.value;
        let model = state.model;
        let peers = Array.from(state.connections.keys());
        await send(pc, 'SYNC_MODEL_DATA_____', JSON.stringify({model: model, peers: peers}));
    });

    pc.on('data', data => {

        if (!(pc as any).initiator && ReactSimplePeerState.value.connections.size === 0) {
